...
class XarrayTreeNode():
    ...
    def __init__(self, name: str, data: xr.Dataset = xr.Dataset(), parent: XarrayTreeNode = None) -> None:
        ...
        # node tree linkage
        self._parent: XarrayTreeNode = None  # will set to parent below
    ...
    @property
    def path(self) -> str:
        """ Return /path/to/this/node from the root node.
        
        Note that the name of the root node is excluded.
        """
        return self.path_from(self.root())
    
    def path_from(self, ancestor: XarrayTreeNode) -> str:
        """ Return /path/to/this/node from a given ancestor node.
        
        Note that the name of the ancestor node is excluded.
        """
        node = self
        path = []
    ...
    def __getitem__(self, path: str) -> XarrayTreeNode:
        """ Return node at path either from the root node (if path starts with /) or otherwise from this node. """
        if path.startswith('/'):
            # path from root node (first name in path must be a child of the root node)
            node = self.root()
        else:
            # path from this node (first name in path must be a child of this node)
            node = self
    ...
    def append_data(self, name: str, data: xr.Dataset | xr.DataArray, append_mode: str = 'merge or replace') -> XarrayTreeNode:
        """ Append data as child of this node.

        Optionally replace or merge with existing child node of same name.
        """
        if name not in list(self.children):
            return XarrayTreeNode(name, data, parent=self)
        
        # child node with same name already exists...

        if append_mode == 'replace':
            # replace existing child node with new node
            self.children.pop(name)
            return XarrayTreeNode(name, data, parent=self)
        
        if append_mode == 'merge' or append_mode == 'merge or replace':
            try:
                # merge data with existing child node (use data for any overlap)
                child: XarrayTreeNode = self.children[name]
                child.data: xr.Dataset = data.combine_first(child.data)
                return child
            except:
    ...
    def rename_vars(self, name_dict: dict[str, str]) -> None:
        ...
        while node is not None:
            ...
            if isinstance(ds, xr.Dataset):
                ...
                if node_name_dict:
                    node.data = ds.rename_vars(node_name_dict)
            node = node.next_depth_first()
    ...
    def leaf_data(self, name: str, root: XarrayTreeNode = None) -> xr.DataArray | None:
        ...
        while node_path[0].parent is not None:
            ...
            if node_path[0] is root:
                break
        while node_path:
            ds: xr.Dataset = node_path[0].data
            if (ds is not None) and (name in ds.data_vars):
                break
            else:
                node_path.pop(0)
        ...
        while node is not None:
            ...
            if (ds is not None) and (name in ds.coords):
                ...
                for dim in coord.dims:
                    if coord.sizes[dim] != self.data.sizes[dim]:
                        return xr.DataArray(dims=(name,), data=np.arange(self.data.dims[name]))
                return coord
            node = node.parent
        return xr.DataArray(dims=(name,), data=np.arange(self.data.dims[name]))
    ...
    def inherit_coords_in_subtree(self) -> None:
        node: XarrayTreeNode = self
        while node is not None:
            node.inherit_coords()
            node = node.next_depth_first()
    ...
    def to_zarr(self, filepath: str = None) -> None:
        ...
        node = self
        while node is not None:
            group_path = node.path_from(self)
            node.data.to_zarr(filepath, group=group_path, mode='w')
            node = node.next_depth_first()
    
    # datatree compatibility

    def to_datatree(self, parent: DataTree = None) -> DataTree:
        node: DataTree = DataTree(name=self.name, data=self.data, parent=parent)
        for child in self.children.values():
    ...
    #     return self.append_data(result_name, result, append_mode)
    ...
    #     permutation_coords = permutations(permutation_coords)
    ...
    #     return self.append_data(result_name, fit, append_mode)
    

# i/o

def open_zarr(filepath: str, parent_group: zarr.hierarchy.Group = None, parent_node: XarrayTreeNode = None) -> XarrayTreeNode:
    if parent_group is None:
        parent_node = XarrayTreeNode(os.path.basename(filepath))
        parent_node.data = xr.open_zarr(filepath, consolidated=False)
        parent_group = zarr.open_group(filepath)
    for name, group in parent_group.groups():
        node = XarrayTreeNode(name, parent=parent_node)
        node.data = xr.open_zarr(filepath, group=group.path, consolidated=False)
        if group.groups():
            XarrayTreeNode.open_zarr(filepath, group, node)
    return parent_node


# datatree compatibility

def from_datatree(dtnode: DataTree, parent: XarrayTreeNode = None) -> XarrayTreeNode:
    node = XarrayTreeNode(name=dtnode.name, data=dtnode.to_dataset(), parent=parent)
    for child in dtnode.children.values():
        XarrayTreeNode.from_datatree(child, node)
    return node
...
def test_tree():
    ...
    root_node = XarrayTreeNode(name='raw data', data=root_ds)
    baselined_node = XarrayTreeNode(name='baselined', data=baselined_ds, parent=root_node)
    ...
    # result_current = result_node._data['current']
    # scaled_current = scaled_node.inherited_data('current')
    ...
    # result_voltage = result_node._data['voltage']
